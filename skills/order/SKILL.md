...
Delivery Coordinationの語彙（Proposal、Design Confirmation、Implementation Authorization、Order、Review Boundary、Worklog Binding）は shared-rules の `modules/delivery-coordination/CONTEXT.md` を正本とする。Worklogの配置・識別・引き渡し・保持の共通契約は shared-rules の `docs/worklogs.md` を正本とし、本skillはorder固有のtask directory確定、`order.md`、kind別のCLI permission準備、起動、worker例外、検収だけを定める。設計への合意はImplementation Authorizationを含まず、本skillは明示的な実装依頼と実行計画の承認が揃ったときだけ起動する。
...
- Claude Code (`kind = "claude"`): 検証済みの利用者引数へ必須引数`--add-dir "$worklog_dir"`を合成したものを`final_args`とする。permission modeとsandboxは利用者の設定を引き継ぎ、別modeへ黙って変更しない。`--dangerously-skip-permissions`、`--allow-dangerously-skip-permissions`、`--permission-mode bypassPermissions`（`=`形式も含む）は起動前に拒否する。追加directoryにも現在のpermission modeが適用される（[Claude Code Permissions](https://code.claude.com/docs/en/permissions#working-directories)）。
- Cursor (`kind = "cursor"`): `--add-dir`、`--sandbox`、`--trust`に対応したCursor Agent CLIを使う。検証済みの利用者引数へ必須引数`--add-dir "$worklog_dir" --sandbox enabled --trust`を合成したものを`final_args`とする。`--trust`は新しいworktreeで起動時に出るworkspace信頼の確認を省く。確認中に届いたpromptは処理されず`agent_prompt_stalled`になるため必須とする。既存の`--trust`は矛盾しない。`--force` / `-f` / `--yolo`、および`enabled`以外の`--sandbox`（`=`形式も含む）は起動前に拒否する。既存の`--sandbox enabled`は矛盾しない。HerdrがCursor Agentを起動するためnative引数に`agent`を重ねない。CLIが必須optionを受理しなければ起動失敗として停止し、sandbox無効化や別kindへfallbackしない。公式の[CLI Parameters](https://cursor.com/docs/cli/reference/parameters)と、導入済みAgent CLIの`--help`を正とする。
